...
handlers.index = function (data, callback) {
    ...
    if (data.method == 'get') {
        ...
        const templateData = {
            'head.title': 'URL Uptime Monitoring',
            'head.description': 'Simple uptime monitoring for HTTP/HTTPS sites all kinds. When your site goes down, we\'ll send you a text to let you know',
            'body.title': 'URL Uptime Monitoring',
            'body.class': 'index'
        };
...
handlers.accountCreate = function (data, callback) {
    ...
    if (data.method == 'get') {
        ...
        const templateData = {
            ...
            'head.description': 'Signeup takes only few secconds.',
                                ...
                                if (checksToDelete > 0) {
                                    ...
                                    userChecks.forEach(function (checkId) {
                                        ...
                                        _data.delete('checks', checkId, function (err) {
                                            ...
                                            checksDelated++;
                                            if (checksDelated == checksToDelete) {
                                                if (!deletionErrors) {
                                                    callback(200);
                                                } else {
                                                    callback(500, { 'Error': 'Errors encountered while attemting to delete all of the user\'s checks. All checks may not have been deleted from the system successfully.' });
                                                }
                                            }
                                        });
                                    });
...
handlers._tokens.post = function (data, callback) {
    ...
    if (phone && password) {
        ...
        _data.read('users', phone, function (err, userData) {
            _performance.mark('user lookup complite');
            if (!err && userData) {
                // Hash the sent password and compare it to the password stored in the user object.
                _performance.mark('begining password hashing');
                const hashedPassword = helpers.hash(password);
                _performance.mark('password hashing complite');
                if (hashedPassword == userData.hashedPassword) {
                    ...
                    _data.create('tokens', tokenId, tokenObject, function (err) {
                        _performance.mark('storing token complite');
                        // Gether all the measurements.
                        _performance.measure('Beginning to End: ', 'entered function', 'storing token complite');
                        _performance.measure('Validating User Input: ', 'entered function', 'inputs validated');
                        _performance.measure('User Lookup: ', 'begining user lookup', 'user lookup complite');
                        _performance.measure('Password Hashing: ', 'begining password hashing', 'password hashing complite');
                        _performance.measure('Token Data Creation: ', 'creating data for token', 'begining storing token');
                        _performance.measure('Token Storing: ', 'begining storing token', 'storing token complite');
...
// Container for all the chacks metods.
handlers._checks = {};
                            ...
                            dns.resolve(hostName, function (err, records) {
                                if (!err && records) {
                                    ...
                                    _data.create('checks', checkId, checkObject, function (err) {
                                        if (!err) {
                                            // Add the check is to the user's object.
                            ...
                            if (!err) {
                                ...
                                _data.read('users', checkData.userPhone, function (err, userData) {
                                    if (!err && userData) {
                                        ...
                                        callback(500, { 'Error': 'Could not find the user who created the chack, so could not remove the check from the list of checks on the user object' });
                                    }
                                });
                            } else {
